...
class TestEnvironmentInit:
    ...
    def test_defaults_to_manual_mode(self):
        """New environment should default to manual mode."""
        env = Environment()
...
class TestSetManualMode:
    ...
    def test_disable_manual_mode(self):
        """Setting manual mode to False should disable it."""
        env = Environment()
        env.set_manual_mode(False)
        assert env._manual_mode is False

    def test_enable_manual_mode(self):
        """Re-enabling manual mode after disabling should work."""
        env = Environment()
        env.set_manual_mode(False)
...
class TestExecuteManualMode:
    ...
    def test_returns_valid_reward(self, monkeypatch):
        """Execute should return the parsed reward value."""
        env = Environment()
    ...
    def test_prints_prompt_and_task(self, monkeypatch, capsys):
        """Execute should print the prompt and task to stdout."""
        env = Environment()
    ...
    def test_accepts_negative_reward(self, monkeypatch):
        """Negative reward within range should be accepted."""
        env = Environment()
    ...
    def test_accepts_boundary_min(self, monkeypatch):
        """Reward at -1.0 boundary should be accepted."""
        env = Environment()
...
class TestExecuteNonManualMode:
    ...
    def test_raises_not_implemented(self):
        """Non-manual execute should raise NotImplementedError."""
        env = Environment()
        env.set_manual_mode(False)
...
class TestParseReward:
    ...
    def test_valid_zero(self):
        """Zero reward string should parse to 0.0."""
        assert Environment._parse_reward("0.0") == 0.0

    def test_valid_positive(self):
        """Positive reward string should parse correctly."""
        assert Environment._parse_reward("0.42") == 0.42

    def test_valid_negative(self):
        """Negative reward string should parse correctly."""
        assert Environment._parse_reward("-0.99") == -0.99

    def test_integer_string(self):
        """Integer string should parse to float."""
